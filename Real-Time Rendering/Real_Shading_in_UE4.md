...
这里补充描述一下Disney的方案吧，也就是被诟病视效不明显但是运算负载巨大：
...
NDF的抽象定义也许让人感觉难以理解，需要注意的是，微表面法线分布函数，本身并非概率密度函数也不是正态分布函数，而是用于描述一片区域(材料)法线的密度特性。众所周知，一块区域当然并非有有限个法线，法线分布函数自然不能用具体的法线向量来描述，而是对于一片微分区域$dA$，将发现位于立体角$\omega_h$内所有微表面的总面积进行求积：

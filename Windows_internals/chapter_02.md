...
KMCS: Kernel Mode Code Signing,仅允许加载由开发人员发布和经过数字签名的设备驱动程序，这些开发人员已经过为数不多的证书颁发机构 (CA) 之一的审查。
...
### Windows 10和Win32k.sys

Win32K.sys的功能被划分为几个内核模块，因此并非所有模块都是特定系统所必需的。这通过降低代码的复杂性和消除许多遗留部分，大大减少了窗口管理器的攻击面。以下是一些例子：

* 在手机（Windows Mobile 10）上，Win32k.sys加载Win32kMin.sys和Win32kBase.sys。
* 在全桌面系统上，Win32k.sys加载Win32kBase.sys和Win32kFull.sys。
* 在某些物联网系统上，Win32k.sys可能只需要Win32kBase.sys。

应用程序调用标准USER函数在显示器上创建用户界面控件，如窗口和按钮。窗口管理器将这些请求传达给GDI，GDI将这些请求传递给图形设备驱动程序，在那里为显示设备格式化这些请求。
GDI提供了一组标准的二维函数，让应用程序在不了解图形设备的情况下与图形设备通信。GDI函数在应用程序和图形设备（如显示器驱动程序和打印机驱动程序）之间进行中介。GDI解释应用程序对图形输出的请求，并将请求发送到图形显示驱动程序。它还为应用程序提供了使用不同图形输出设备的标准接口。该接口使应用程序代码能够独立于硬件设备及其驱动程序。GDI根据设备的功能调整消息，通常将请求划分为可管理的部分。

> 在最初的Windows子系统设计中，子系统进程（Csrss.exe）负责管理控制台窗口，每个控制台应用程序（如命令提示符Cmd.exe）都与Csrss.exe通信。从Windows 7开始，系统上的每个控制台窗口都使用一个单独的进程：控制台窗口主机（Conhost.exe）。
> 在Windows8及更高版本中，控制台体系结构再次发生了变化。Conhost.exe进程仍然存在，但现在是由控制台驱动程序（\Windows\System32\Drivers\ConDrv.sys）从基于控制台的进程（而不是像Windows 7中那样从Csrss.exe）派生出来的。该进程通过发送读、写、I/O控制和其他I/O请求类型，使用控制台驱动程序与Conhost.exe通信。Conhost.exe被指定为服务器，使用控制台的进程是客户端。
